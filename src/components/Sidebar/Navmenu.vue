<template>
  <ul>
    <li
      ...
      ${this.$route.path === item.link ? 'menu-item-active' : ''}
...
export default {
  ...
  created() {
    ...
    this.items.map((item, i) => {
      item.child?.map((ci) => {
        if (ci.childlink === router.currentRoute.value.path) {
...
.single-sidebar-menu {
  @apply relative;
  .menulabel {
    @apply text-slate-800 dark:text-slate-300 text-[11px] sm:text-xs font-semibold uppercase mb-4 mt-4;
  }
  > .menu-link {
    @apply flex text-slate-600 font-medium dark:text-slate-300 text-base sm:text-sm capitalize px-3 py-3 rounded-[4px] cursor-pointer;
  }
  .menu-icon {
    @apply icon-box inline-flex items-center text-slate-600 dark:text-slate-300 text-xl sm:text-lg mr-3;
  }
}
